<script lang="ts">
  ...
  const handleSubmit = async (e: Event) => {
    ...
    const response = await fetch(`${PUBLIC_API_URL}/users/${$userData.id}`, {
      method: "PUT",
      credentials: "include",
      headers: {
      ...
      body: JSON.stringify({
        user: {
          ...
          name: userName,
          email: user.email,
          avatar: image_to_upload ?? user.avatar,
          pwd: userPassword,
          role: $userData.role,
        },
      }),
    ...
    {#if user}
      ...
      <Tabs style="underline">
        <TabItem open>
          ...
          <div>
            <form class="flex flex-col" on:submit={handleSubmit} id="editUser">
              ...
              <Button class="self-end" type="submit" form="editUser"
                >Apply</Button
              >
            </form>
          </div>
        ...
        <TabItem>
          ...
          <div class="flex justify-between">
            <P class="flex items-center">Connect with an external account</P>
            <Button color="green" on:click={() => (connectionsModal = true)}
